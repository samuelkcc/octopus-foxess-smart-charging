...
assert.match(stylesSource, /@media \(max-width: 1100px\)/, 'The two-column dashboard must stack before either tariff chart becomes too narrow');
assert.match(stylesSource, /\.tariff-chart-panel \{[^}]*overflow: hidden/, 'Each chart panel must contain its canvas');
assert.match(stylesSource, /\.tariff-chart-frame canvas \{[^}]*width: 100% !important/, 'Chart canvases must resize to their current column');
...
assert.match(markupSource, /id="live-ws-policy"/);
assert.match(markupSource, /value="on-demand">Live WS on demand/);
assert.match(markupSource, /value="always">Always Live WS \(REST fallback\)/);
assert.match(markupSource, /value="rest">Official REST only/);
assert.equal((markupSource.match(/>☰ Menu<\/button>/g) || []).length, 1, 'The dashboard must show one Menu button');
assert.match(markupSource, /id="section-smart-discharging"/, 'Smart Discharging must be a separate dashboard block');
assert.match(markupSource, /id="toggle-weekly-discharge"/, 'Smart Discharging must support a scheduled rule');
assert.match(markupSource, /id="toggle-discharge-pause-dispatch" checked/, 'Smart Dispatch protection must default on');
assert.match(markupSource, /id="toggle-discharge-overrides-charge" checked/, 'Forced Charge conflict protection must default on');
assert.match(markupSource, /id="show-section-intelligentOctopus"/);
assert.match(markupSource, /id="show-section-foxessMode"/);
assert.match(markupSource, /id="show-section-deviceSettings"/);
assert.match(markupSource, /id="show-section-smartCharging"/);
assert.match(markupSource, /id="show-section-smartDischarging"/);
assert.doesNotMatch(markupSource, /mini-viewer-hud|Status HUD/, 'Mini HUD markup must be removed');
assert.doesNotMatch(appSource, /toggleMiniViewer|updateMiniViewer/, 'Mini HUD code must be removed');
assert.doesNotMatch(markupSource, /📊 Live Telemetry/, 'The top energy overview replaces the duplicated telemetry panel');
assert.match(markupSource, /Home Energy Protection[\s\S]*?fox-live-source-badge[\s\S]*?Intelligent Octopus Go/, 'The Live WS source must stay in the top protection overview');
...
assert.match(markupSource, /styles\.css\?v=2026\.8\.1\.3/);
assert.match(markupSource, /app\.js\?v=2026\.8\.1\.3/);
